...
def Revenue():
    ...
    prev_day_orders = RevenueRep.GetOrdersPrevDay()
    ...
    for order in prev_day_orders:
        prev_total_revenue += order.total

    percentage_today_revenue_with_prev_day = round((today_total_revenue / prev_total_revenue), 4)
    ...
    percentage_month_revenue_with_prev_month = round((month_total_revenue / prev_month_total_revenue), 4)

    grow_percentage_to_prev_day = 1 if prev_total_revenue == 0 else round((today_total_revenue / prev_total_revenue) - 1)
    ...
    return RevenueRep.GetTotalRevenueToday()
